...
-export([eventsource_event_list/1]).
-export([eventsource_event_dataline/1]).
...
groups() ->
	BaseTests = [
		...
		eventsource_comment,
		eventsource_event,
		eventsource_event_list,
		eventsource_event_dataline
	],
		...
		{"localhost", [
			...
			{"/eventsource_event", eventsource_handler,
				[{send, {event, [{id, <<"id">>}, {type, <<"type">>},
					{data, <<"line 1\nline 2">>}]}}, quit]},
			{"/eventsource_event_list", eventsource_handler,
				[{send, [{event, [{data, <<"1">>}]},
					{event, [{data, <<"2">>}]}]}, quit]},
			{"/eventsource_event_dataline", eventsource_handler,
				[{send, {event, [{dataline, <<"1">>}]}}, quit]}
		]}
	]).

eventsource_init(Config) ->
	%% eventsource:init/1 sends the correct header
...
eventsource_last_event_id(Config) ->
	%% eventsource:last_event_id/1 retrieves the correct header
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
...
eventsource_retry(Config) ->
	%% eventsource:send/2 sends correct retry frames
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
...
eventsource_comment(Config) ->
	%% eventsource:send/2 sends correct comment frames
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
...
eventsource_event(Config) ->
	%% eventsource:send/2 sends correct event frames
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
...
eventsource_event_list(Config) ->
	%% eventsource:send/2 sends correct events passed as a list
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
		"GET /eventsource_event_list HTTP/1.1\r\n"
		"Host: localhost\r\n"
		"\r\n"),
	{ok, Response} = recv(Socket),
	{ok, {http_response, {1, 1}, 200, _}, Rest}
...
eventsource_event_dataline(Config) ->
	%% eventsource:send/2 sends correct events passed as a list
	{port, Port} = lists:keyfind(port, 1, Config),
	{ok, Socket} = gen_tcp:connect("localhost", Port,
		[binary, {active, false}, {packet, raw}]),
	ok = gen_tcp:send(Socket,
		"GET /eventsource_event_dataline HTTP/1.1\r\n"
		"Host: localhost\r\n"
		"\r\n"),
	{ok, Response} = recv(Socket),
	{ok, {http_response, {1, 1}, 200, _}, Rest}
