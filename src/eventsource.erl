...
%%
%% Call <em>eventsource:init/1</em> from the loop handler's
...
%% reconnecting.
%%
%% Use <em>eventsource:last_event_id/1</em> to obtain the
%% last event id seen by the client in order to send events it has
%% missed.
%%
%% Use <em>eventsource:send/2</em> to send events from the
